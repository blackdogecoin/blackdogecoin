...
const string strMessageMagic = "Blackdogecoin Signed Message:\n";
...
{
    int64_t nSubsidy = 99206.3 * COIN;

    if (pindexBest->nHeight == 1)
    {
      nSubsidy = 15000000 * COIN; // 1.5% Premine | 15M
...
static const int64_t nTargetTimespan = 10 * 60;  // 1 min
//
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Blackdogecoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "private keys for compressed and uncompressed bitcoin public keys use the same version byte";
        CTransaction txNew;
        txNew.nTime = 1404340370;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1404340370;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1915846;
		if(fTestNet)
        {
            block.nNonce   = 0;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x189d6ee9eeb6269ec6ed280caa729c32a47f90747df673d3bae7a6b7fceac785"));
        block.print();
